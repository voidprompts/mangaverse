...
export async function fetchChapterPages(chapterId){
  ...
  try{
    ...
    return{pages:saver.map(function(f,i){
      var imgUrl=baseUrl+'/data-saver/'+hash+'/'+f;
      var imgUrlHQ=baseUrl+'/data/'+hash+'/'+(hq[i]||f);
      return{index:i+1,url:PROXY+encodeURIComponent(imgUrl),urlHQ:PROXY+encodeURIComponent(imgUrlHQ)};
    }),total:saver.length};
